/* ================= PROJECT DETAIL PAGE =================
   One page serves all twelve developments, selected by ?p=<slug>. The
   alternative — twelve near-identical HTML files — would mean twelve copies
   of the same layout to keep in step, so the layout is authored once in
   project.html and the content lives here.

   Note the trade-off this accepts: the card grid in projects.html is authored
   as static HTML (so browsing works with no JavaScript), which means a
   project's name, type, status and location exist in both places. Slug is the
   join. Without a build step there is no way to have one source of truth AND
   a no-JS grid; the grid won that argument because it is the page people
   actually land on. If these ever disagree, projects.html is the browse view
   and this file is the detail — fix them together. */
(function(){
  ...
  const PROJECTS = [
    ...
    },
    {
      ...
      units: {
        ...
        rows: [
          ['3rd–8th', '66,000', '11,000 sq ft', 'Office'],
          ['9th–14th', '66,000', '11,000 sq ft', 'Office'],
          ['15th–18th', '44,000', '11,000 sq ft', 'Office / fit-out ready']
        ]
      },
      floorPlans: [
    ...
    },
    {
      ...
      units: {
        columns: ['Floor Range', 'Size (sq ft)', 'Floorplate', 'Use'],
        rows: [
          ['Ground–2nd', '48,000', '16,000 sq ft', 'Retail'],
          ['3rd–10th', '88,000', '11,000 sq ft', 'Workspace']
        ]
      },
      floorPlans: [
    ...
    },
    {
      ...
      units: {
        ...
        rows: [
          ['Block 1–2', '80,000', '9.5 m', 'Light industrial'],
          ['Block 3–4', '80,000', '9.5 m', 'Light industrial'],
          ['Block 5–6', '80,000', '9.5 m', 'Light industrial']
        ]
      },
      floorPlans: [
    ...
    },
    {
      ...
      units: {
        ...
        rows: [
          ['Warehouse', '210,000', '22', 'Distribution']
        ]
      },
      floorPlans: [
    ...
    },
    {
      ...
      units: {
        columns: ['Floor Range', 'Size (sq ft)', 'Floorplate', 'Use'],
        rows: [
          ['3rd–12th', '125,000', '12,500 sq ft', 'Office'],
          ['13th–22nd', '125,000', '12,500 sq ft', 'Office']
        ]
      },
      floorPlans: [
    ...
    },
    {
      ...
      units: {
        columns: ['Floor Range', 'Size (sq ft)', 'Floorplate', 'Use'],
        rows: [
          ['Ground–2nd', '62,000', '31,000 sq ft', 'Retail'],
          ['3rd–9th', '80,000', '11,400 sq ft', 'Office']
        ]
      },
      floorPlans: [
  ...
  if(!project){
    // Unknown or missing slug: say so and offer the way back, rather than
    // rendering a shell of empty fields.
    root.hidden = true;
    const missing = el('projectMissing');
    if(missing) missing.hidden = false;
    document.title = 'Project not found | RHL Properties Ltd';
    return;
  }

  document.title = project.name + ' | RHL Properties Ltd';
  const meta = document.querySelector('meta[name="description"]');
  if(meta) meta.setAttribute('content', project.summary);

  setText('pdName', project.name);
  setText('pdSummary', project.summary);
  setText('pdMeta', project.type + ' · ' + project.location);
  const badge = el('pdStatus');
  if(badge){ badge.textContent = project.status; badge.className = 'pcard-status is-' + project.status.toLowerCase(); }

  const heroMedia = el('pdHeroMedia');
  if(heroMedia) heroMedia.style.backgroundImage = `url("${project.hero}")`;

  // Facts
  const factsEl = el('pdFacts');
  if(factsEl){
    factsEl.innerHTML = '';
    Object.keys(project.facts).forEach((key) => {
      const item = document.createElement('div');
      item.className = 'fact';
      item.innerHTML = `<dt>${key}</dt><dd></dd>`;
      item.querySelector('dd').textContent = project.facts[key];
      factsEl.appendChild(item);
    });
  }

  // Body copy
  const bodyEl = el('pdBody');
  if(bodyEl){
    bodyEl.innerHTML = '';
  ...
  // Progress + stage timeline — only meaningful while a project is actually
  // under construction. Thresholds are a simple read of the percentage, not
  // separately authored data, since every ongoing project already reports
  // one number and a second parallel "stage" field would just be a place for
  // the two to disagree.
  const progressWrap = el('pdProgress');
  if(progressWrap){
    if(typeof project.progress === 'number'){
      progressWrap.hidden = false;
      setText('pdProgressValue', project.progress + '% complete');
      const bar = el('pdProgressBar');
      if(bar){
        bar.style.width = project.progress + '%';
        bar.parentElement.setAttribute('aria-valuenow', String(project.progress));
      }

      const stages = ['Foundation', 'Structure', 'Finishing', 'Handover'];
      const thresholds = [30, 65, 90, 100];
      const currentIdx = thresholds.findIndex((t) => project.progress <= t);
      const stageTrack = el('pdStageTrack');
      if(stageTrack){
        stageTrack.innerHTML = '';
        stages.forEach((label, i) => {
          const stage = document.createElement('div');
          stage.className = 'pd-stage' + (i < currentIdx ? ' is-done' : i === currentIdx ? ' is-current' : '');
          stage.innerHTML = `<div class="pd-stage-bar"></div><span class="pd-stage-label">${label}</span>`;
          stageTrack.appendChild(stage);
        });
      }
    } else {
  ...
  // Amenities
  const amenitiesEl = el('pdAmenities');
  if(amenitiesEl){
    amenitiesEl.innerHTML = '';
    (project.amenities || []).forEach((text) => {
  ...
  // Unit information table — hidden entirely for the few projects (none
  // currently, but the guard costs nothing) that carry no unit breakdown.
  const unitsSection = el('pdUnitsSection');
  if(unitsSection){
    if(project.units && project.units.rows.length){
      unitsSection.hidden = false;
      const head = el('pdUnitsHead');
      const body = el('pdUnitsBody');
      head.innerHTML = project.units.columns.map((c) => `<th>${c}</th>`).join('');
      body.innerHTML = '';
      project.units.rows.forEach((row) => {
  ...
  if(galleryEl){
    ...
    project.gallery.forEach((src, i) => {
      ...
      image.src = src;
      image.alt = `${project.name} — view ${i + 1}`;
      image.loading = 'lazy';
  ...
  // Floor plans — each thumbnail is a lightbox trigger (see lightbox.js),
  // not a link, so opening one never costs the visitor their scroll position.
  const floorplansEl = el('pdFloorplans');
  if(floorplansEl){
    floorplansEl.innerHTML = '';
    (project.floorPlans || []).forEach((plan) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'pd-floorplan';
      btn.setAttribute('data-lightbox', plan.img);
      btn.setAttribute('data-caption', `${project.name} — ${plan.label}`);
      btn.innerHTML = `<img src="${plan.img}" alt="${project.name} — ${plan.label}" loading="lazy"><span>${plan.label}</span>`;
      floorplansEl.appendChild(btn);
    });
  }

  // Location map — same Google Maps embed pattern as the homepage, queried
  // with the project's own area rather than the head office address.
  const mapFrame = el('pdMapEmbed');
  if(mapFrame){
    const mapQuery = `${project.name}, ${project.location}, Dhaka, Bangladesh`;
    mapFrame.src = `https://www.google.com/maps?q=${encodeURIComponent(mapQuery)}&output=embed`;
  }

  // Brochure — one shared placeholder PDF until the client supplies
  // per-project brochures (see TASKS.md "Blocked on client").
  const brochure = el('pdBrochure');
  if(brochure) brochure.setAttribute('download', `RHL-${project.slug}-brochure.pdf`);

  // Inline enquiry form — pre-fill rather than lock, so a visitor comparing
  // two developments can still adjust it before sending.
  const projectField = el('pd-f-project');
  if(projectField) projectField.value = project.name;
  const enquiryForm = el('pdEnquiryForm');
  if(enquiryForm) enquiryForm.dataset.success = `Thank you — your enquiry about ${project.name} has been received. Our team usually replies within two working days.`;

  /* Prev / next follow the authored order, wrapping at both ends, so the
     controls are never dead on the first or last project. */
  const index = PROJECTS.indexOf(project);
  const prev = PROJECTS[(index - 1 + PROJECTS.length) % PROJECTS.length];
  const next = PROJECTS[(index + 1) % PROJECTS.length];
  function wire(id, target, label){
    const link = el(id);
    if(!link) return;
    link.href = 'project.html?p=' + target.slug;
    link.querySelector('.pd-nav-name').textContent = target.name;
    link.setAttribute('aria-label', label + ': ' + target.name);
  }
  wire('pdPrev', prev, 'Previous development');
  wire('pdNext', next, 'Next development');

  // Related: same location first, then same type, capped at three.
  const relatedEl = el('pdRelated');
  if(relatedEl){
    const scored = PROJECTS.filter((p) => p !== project)
      .map((p) => ({ p, score: (p.location === project.location ? 2 : 0) + (p.type === project.type ? 1 : 0) }))
    ...
    scored.forEach((p) => {
      ...
      card.innerHTML =
        `<div class="pcard-media"><img loading="lazy" alt=""></div>` +
        `<span class="pcard-status is-${p.status.toLowerCase()}"></span>` +
        `<div class="pcard-body"><span class="pcard-meta"></span><h3><a class="pcard-link"></a></h3><p></p></div>`;
      const image = card.querySelector('img');
      image.src = p.hero; image.alt = p.name;
      card.querySelector('.pcard-status').textContent = p.status;
      card.querySelector('.pcard-meta').textContent = p.type + ' · ' + p.location;
      const link = card.querySelector('.pcard-link');
      link.href = 'project.html?p=' + p.slug; link.textContent = p.name;
      card.querySelector('.pcard-body p').textContent = p.summary;
      relatedEl.appendChild(card);
    });
  }

  // The whole page just changed height and gained images.
