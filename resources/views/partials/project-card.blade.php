...
<article class="pcard" data-status="{{ $project->status_slug }}" data-type="{{ strtolower($project->type) }}" data-location="{{ strtolower($project->location) }}" data-slug="{{ $project->slug }}">
  <div class="pcard-media"><img src="{{ $project->hero_image_url }}" alt="{{ $project->name }}" loading="lazy"></div>
  <span class="pcard-status is-{{ $project->status_slug }}">{{ $project->status }}</span>
